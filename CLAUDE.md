...
2. **Elevation enrichment** (`elevation.py`): query the `ElevationService` for elevation at each node. Sample at `elevation_sample_interval_m` intervals along ways. Cache results within a request. See **Elevation data** section for source details.
...
- **Bridge/tunnel handling**: ways tagged `bridge=yes` or `tunnel=yes` are treated as straight-line segments. Intermediate shape points are ignored. Grade is computed from start-node elevation to end-node elevation over straight-line distance. Tunnels may optionally be excluded entirely via a toggle (post-MVP).
...
- **Detection roads (non-traversable)**: the "avoid bigger roads" toggle can only stop a descent at a road that exists in the graph. Bigger roads are normally outside the rideable set (`DEFAULT_ROAD_TYPES` tops out at `tertiary`), so they would never be fetched and a descent would cross straight through them (e.g. 16th Ave crossing Geary). To fix this, the Overpass query always fetches the tiers in `DETECTION_ROAD_TYPES` (`secondary`–`motorway`) in addition to the rideable set. These ways are added to the graph with edge attribute `traversable=False`: pathfinding never expands onto them (so they're never *ridden*), but their presence at a junction still triggers the bigger/equal-road hard stop via successor ranks. Traversability is decided in `main.py` (`highway ∈ road_types`, within `max_road_rank`, surface allowed) and carried onto edges by `graph.py`.
